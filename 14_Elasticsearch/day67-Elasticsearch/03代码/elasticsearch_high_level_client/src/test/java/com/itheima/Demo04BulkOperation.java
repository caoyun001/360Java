...
public class Demo04BulkOperation {
    ...
    public void bulkCreate() throws IOException {
        ...
        //新增文档
        request.add(new IndexRequest().id("1").type("_doc").index("heima1").source(XContentType.JSON,"title","小米","price",2699));
        request.add(new IndexRequest().id("2").type("_doc").index("heima1").source(XContentType.JSON,"title","大米","price",3699));
        request.add(new IndexRequest().id("3").type("_doc").index("heima1").source(XContentType.JSON,"title","超米","price",19999));
        //发送请求，获取响应对象
    ...
    public void bulkDelete() throws IOException {
        BulkRequest request = new BulkRequest();
        //新增文档
        request.add(new DeleteRequest().id("1").type("_doc").index("heima1"));
        request.add(new DeleteRequest().id("2").type("_doc").index("heima1"));
        request.add(new DeleteRequest().id("3").type("_doc").index("heima1"));
        //发送请求，获取响应对象
        BulkResponse response = client.bulk(request, RequestOptions.DEFAULT);
        //打印响应结果信息
        System.out.println("took::"+response.getTook());
